...
- **DEC-19 (Commodity-as-endpoint, deferred):** Tiku's `REL_1649_CLAIMS_COMMODITY_MONOPOLY` and `REL_1740_CLAIMS_MONOPOLY_SALT` use `object_id` values (`COMMODITY_PEPPER`, `COMMODITY_SALT`) that are not Actor/Location ids. A projection must not silently treat Commodity as a graph node just to make these two edges resolve — that would pre-empt DEC-19's still-open option (a)/(b) choice by a side effect of graph-building. These two relations stay unprojected (or projected with an explicit `unresolved_endpoint_type=COMMODITY` flag, never as a plain edge) until DEC-19 is resolved.
