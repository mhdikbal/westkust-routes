...
Nothing else in the first draft is affected by these two corrections; sections not discussing restraint devices or the 372 figure are unchanged from the approved-pending-revision draft.
...
| C10 | **(Revised — was wrongly stated absent in the first draft.)** The raw DOCX source (`docs/enclave/enklave-salido.docx`, `word/document.xml`) explicitly records two restraint-device entries, in the same two ammunition-inventory sections as C9's weapons: `1 belenggu dengan lima gelang dan satu kunci` (Ammunitie van oorlogh, main store) and `1 belenggu dengan tiga gelang dan satu kunci` (Waker transfer, Beneden-Pagger, 6 Jan 1682, Hoffman→Pleijtner). **Neither entry exists in `10_inventory_items.csv` or `00_source_passages.csv`** — a confirmed canonical-extraction gap, not a disagreement about interpretation. | direct DOCX `word/document.xml` text extraction and read, this revision |
...
Both entries are routed to a **required canonical-extraction audit** (§5 `restraint_device_review.csv`, §15) rather than silently added to `10_inventory_items.csv` by this plan (canonical datasets are not modified by this document) or silently dropped as the first draft did. Until that audit and an image-review pass complete, both entries carry `extraction_audit_required=true` and `image_review_required=true`, and no UI built on this schema may present them as fully reviewed.
...
One row per restraint-device-relevant source passage. This is the audit/review workbench for C10, and the technical home of the P6 prohibition.

```
review_id, source_document_id, source_passage_id (blank until extraction audit assigns one),
...
canonical_extraction_status (missing_from_canonical_csv|extracted|extraction_rejected),
extraction_audit_required (bool, default true),
image_review_required (bool, default true),
reviewer, review_date, reviewer_note
```
**Seed content proposed (not written to any file by this plan — for the future implementation to populate after audit):** two rows, one per C10 entry, both starting with `canonical_extraction_status=missing_from_canonical_csv`, `extraction_audit_required=true`, `image_review_required=true`, `actual_use_status=not_recorded`, `target_person_status=not_recorded`, `date_of_use_status=not_recorded`. Neither row should have `canonical_extraction_status` changed to `extracted` until a human reviewer confirms the DOCX reading against the original scan/folio image (per `docs/SOURCE_PROVENANCE.md`'s existing image-verification fields) and a decision is made about whether/how to add the corresponding rows to a *future* canonical CSV release (out of scope for this plan — see §17).
...
**Phase A0 — Required audits (new, gates Phase A).** Two audits must complete, or be explicitly deferred with documented risk acceptance by a reviewer, before Phase A's derivation layer computes anything downstream of them:
  - **Canonical-extraction audit for the two restraint rows** (C10): confirm the DOCX reading against the original archival scan/folio image where available (`docs/SOURCE_PROVENANCE.md` image-verification fields), assign `source_passage_id`s, and decide whether/how the two rows are added to a *future* canonical CSV release (a separate MIG-NNN-style proposal, out of scope for this plan itself — see §17).
  - **Cross-document temporal-overlap review** for the Madagascar cohort and the wider aggregate-group population (§8 tier 6): establish whether any other document in the corpus (or a later/earlier personnel register) describes an overlapping time window for any of the same people, which would affect whether 308 (tier 5) is even the right *single-document* estimate to carry forward, let alone a verified unique-person count.
...
- **Restraint-device tests** (new): `test_restraint_device_review_seed_rows_require_extraction_audit` (both rows must load with `extraction_audit_required=true`), `test_restraint_device_actual_use_status_only_accepts_not_recorded` (schema-level enforcement of P6 until Phase A0 changes it), `test_coercion_evidence_restraint_device_row_requires_completed_audit` (a `coercion_evidence.csv` row with `evidence_type=restraint_device` cannot be created while its corresponding `restraint_device_review.csv` row still has `extraction_audit_required=true`).
...
- **The two restraint-device rows (C10) are not added to any canonical CSV by this plan or its future implementation phases as currently scoped.** If Phase A0's extraction audit confirms them (against source imagery, per `docs/SOURCE_PROVENANCE.md`), adding them to a *canonical* release is a separate, future proposal following the existing `V0_4_MIGRATION_PLAN.md`-style MIG-NNN process and would produce a new canonical version (e.g., `v0_4_2` or later) — explicitly out of scope here. Until then, they exist only in `restraint_device_review.csv` within the critical layer, clearly marked as pending.
