...
graph TD
    ...
    S1 -->|S1-03 gate — BLOCKED: archival scans| S2[Sprint 2: Critical dataset candidate v0.5]
...
- **DoD**: `A0-6` (Workstream 1 reviewer sign-off) **and** `A0-11` (Workstream 2 reviewer sign-off) both recorded in `PHASE_A0_SPRINT_BOARD.md`; `S1-03` gate task closed. **Currently blocked**: Workstream 1 cannot reach `A0-6` until archival scans are obtained (`docs/enclave/scans/` does not exist in this repository) — see External Blockers below. Workstream 2 has no external blocker and can proceed independently.
...
| Blocker | Affects | Current state | Who must resolve it |
|---|---|---|---|
| **Archival scan access** (Nationaal Archief, Den Haag — Access 1.04.02, Inventory 7964) | `S1-01` (and everything downstream that depends on the restraint-device audit reaching a non-pending state: `S2-03` partially, `S2-04` partially, `S4-05` partially) | `docs/enclave/scans/` does not exist in this repository (confirmed during Phase A0 board setup and re-confirmed as unchanged for this board) | Research owner — this is not resolvable by repository-local work |
| **Cross-document corpus review exhaustiveness** | `S1-02` | No blocker on starting, but requires a genuinely exhaustive review of `01_documents.csv`'s date-overlapping documents, not a sampled check, per `A0-7`/`A0-8` acceptance criteria | Reviewer conducting `S1-02` |
...
**The archival-scan blocker is the single most consequential item on this board** — it is the only one Sprint 4 (`S4-05`)'s evidence requirement explicitly notes can be worked around (the Coercion Context section can ship with both restraint-device rows in "audit pending" state), meaning the programme does not have to stall entirely on it, but the *badges* on that section cannot honestly read anything other than "pending" until it resolves.
